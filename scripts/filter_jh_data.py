...
from utils import collapse_data_sets, print_csv
...
def world_data(args):
    ...
    for date in sorted(world_data.keys()):
        daily_data = [str(world_data[date][data_key])
...
def country_data(args):
    ...
    for date in sorted(country_data.keys()):
        daily_data = [str(country_data[date][data_key])
                      for data_key in data_keys]
        data_row = [date] + daily_data
        rows.append(data_row)
...
def state_data(args):
    ...
    for date in sorted(state_data.keys()):
        daily_data = [str(state_data[date][data_key])
                      for data_key in data_keys]
        data_row = [date] + daily_data
        rows.append(data_row)
...
def county_data(args):
    ...
    for date in sorted(county_data.keys()):
        daily_data = [str(county_data[date][data_key])
                      for data_key in data_keys]
        data_row = [date] + daily_data
        rows.append(data_row)
...
def labelled_data(args):
    ...
    for date in sorted(data_sets[col_specs[0]].keys()):
        ...
        for col_spec in col_specs:
            data_fields.append(
                str(data_sets[col_spec][date][col_spec.data_key]))
